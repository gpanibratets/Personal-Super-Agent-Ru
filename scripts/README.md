# 🔧 Utility Scripts

Полезные скрипты для автоматизации проверок и вычислений.

## 📊 check_range.py

Проверяет, попадают ли числа в заданные диапазоны. Полезно для анализов здоровья, финансовых показателей, и других числовых данных.

### Использование:
```bash
python3 scripts/check_range.py [значение] [мин] [макс] [значение] [мин] [макс] ...
```

### Примеры:

**Анализы крови:**
```bash
# Холестерин (норма 120-200), Сахар (норма 70-100), Давление (норма 80-120)
python3 scripts/check_range.py 185 120 200 92 70 100 85 80 120
```

**Бюджетные категории:**
```bash
# Расходы на еду (бюджет 500-700), Развлечения (бюджет 200-300)
python3 scripts/check_range.py 650 500 700 250 200 300
```

**Результат:**
```
🔍 Проверка диапазонов:
==================================================
   185.0 | [ 120.0 -  200.0] | ✅ В диапазоне
    92.0 | [  70.0 -  100.0] | ✅ В диапазоне
    85.0 | [  80.0 -  120.0] | ✅ В диапазоне
==================================================
```

### Зачем это нужно?

LLM часто ошибаются при сравнении чисел. Этот скрипт даёт точный математический результат, который можно использовать в анализе.

## 📤 send_to_telegram.py

Отправляет файлы и сообщения в Telegram прямо из командной строки. Поддерживает несколько чатов с понятными именами.

### Настройка:

1. Создайте бота через @BotFather в Telegram
2. Получите токен бота и Chat ID для каждого чата
3. Запустите скрипт один раз - он создаст файл конфигурации
4. Заполните `scripts/telegram_config.json` с вашими данными

**Подробная инструкция:** См. `scripts/telegram_setup.md` или `scripts/QUICK_START_TELEGRAM.md`

### Использование:

```bash
# Просмотр списка чатов
python3 scripts/send_to_telegram.py --list

# Отправить файл в чат по умолчанию
python3 scripts/send_to_telegram.py /path/to/file.txt

# Отправить файл в конкретный чат
python3 scripts/send_to_telegram.py /path/to/file.txt --chat doctor

# Отправить файл с подписью
python3 scripts/send_to_telegram.py /path/to/file.txt "Результаты анализов" --chat doctor

# Отправить текстовое сообщение
python3 scripts/send_to_telegram.py --text "Привет!" --chat doctor

# Пример: отправить результаты анализов врачу
python3 scripts/send_to_telegram.py \
  Docs/Health/Medical-Records/2025-07-30-blood-test-telegram.txt \
  "Результаты анализов крови от 30.07.2025" \
  --chat doctor
```

### Структура конфигурации:

```json
{
  "bot_token": "ваш_токен",
  "chats": {
    "myself": "chat_id_1",
    "doctor": "chat_id_2",
    "family": "chat_id_3"
  },
  "default_chat": "myself"
}
```

Можно добавить любое количество чатов с понятными именами!

### Безопасность:

- Файл `telegram_config.json` автоматически добавлен в `.gitignore`
- Токен бота не будет закоммичен в Git
- Не делитесь токеном с другими

## Идеи для других скриптов:

- `calculate_bmi.py` - расчёт индекса массы тела
- `currency_convert.py` - конвертация валют
- `compound_interest.py` - расчёт сложных процентов
- `health_metrics.py` - расчёт здоровых диапазонов по возрасту 